...
- `Metascore` arrives as a *string* and can be `"N/A"`; the client parses it into a `*int` so missing stays distinguishable from zero
- Free tier is 1000 requests/day, hence the per-run batch cap (`OMDB_BATCH_SIZE`) rather than a full-library sweep
- Rate limiting, circuit breaker, retry, and key-safe-URL handling live in the client. It takes no logger — pass `omdb.OnRetry(...)` if a retry needs to reach the logs
